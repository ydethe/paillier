    ...
    def test_decrypt_positive_integers(self):
        ...
        for num in numbers:
            with tempfile.NamedTemporaryFile() as encfile:
                ...
                self.runner.invoke(
                    cli, ["encrypt", self.public_keyfile.name, str(num), "--output", fname]
                )

                result = self.runner.invoke(cli, ["decrypt", self.private_keyfile.name, fname])
                assert result.exit_code == 0
            ...
            with tempfile.NamedTemporaryFile() as encfile:
                ...
                with tempfile.NamedTemporaryFile() as outfile:
                    result = self.runner.invoke(
                        cli, ["decrypt", self.private_keyfile.name, fname, "--output", outfile.name]
                    )
                    assert result.exit_code == 0
