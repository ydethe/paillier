...
class PaillierGeneric(unittest.TestCase):
    ...
    def testStaticPrivateKeyConstructor(self):
        ...
        self.assertEqual(
            private_key, private_key_from_static, "The private keys should be the same."
        )
        self.assertEqual(
            private_key_from_static.decrypt(c), 4242, "Result of the decryption should be 4242"
        )
...
class PaillierTestRawEncryption(PaillierTest):
    ...
    def testEncryptIsRandom(self):
        ...
        self.assertNotEqual(enc_num2.ciphertext(False), enc_num3.ciphertext(False))
...
class PaillierTestEncodedNumber(PaillierTest):
    ...
    def testEncodeIntDecodeInt4(self):
        ...
        encoded = self.EncodedNumberCls.encode(self.public_key, self.public_key.max_int)
        self.assertEqual(0, encoded.exponent)
        decoded = encoded.decode()
    ...
    def testEncodeIntTooLargePositive(self):
        ...
        self.assertRaises(
            ValueError, self.EncodedNumberCls.encode, self.public_key, self.public_key.max_int + 1
        )
        self.assertRaises(
            ValueError,
            self.EncodedNumberCls.encode,
            self.public_key,
            2 ** (paillier.DEFAULT_KEYSIZE - 1),
        )
    ...
    def testEncodeFloat0(self):
        ...
        self.EncodedNumberCls.BASE**enc.exponent
...
class PaillierTestEncodedNumberDefaultBase(PaillierTestEncodedNumber):
    ...
    def testManualPrecision0(self):
        ...
        encoding2 = self.EncodedNumberCls.encode(
            self.public_key, decoded + 0.500001 * prec, precision=prec
        )
    ...
    def testAutomaticPrecisionAgreesWithEpsilon(self):
        ...
        for i in range(-floor_happy, floor_happy + 1):
            enc1 = self.EncodedNumberCls.encode(self.public_key, 2.0**i)
            enc2 = self.EncodedNumberCls.encode(self.public_key, 2.0**i, precision=eps * 2**i)
            ...
            enc4 = self.EncodedNumberCls.encode(self.public_key, val, precision=rel_eps)
...
class PaillierTestEncryptedNumber(PaillierTest):
    ...
    def testAutomaticPrecision0(self):
        ...
        assert one_plus_eps > 1.0  # If this is false, we have trouble!
        ...
        ciphertext4 = ciphertext3 + eps * 4.0 / 5
        decryption4 = self.private_key.decrypt(ciphertext4)
    ...
    def testDecreaseExponentTo(self):
        ...
        self.assertAlmostEqual(1.01, self.private_key.decrypt(ciphertext2), places=8)
    ...
    def testAddWithEncryptDecryptFloat0(self):
        # Add 1 to a small negative number
        ciphertext1 = self.public_key.encrypt(-15.0)
        ciphertext2 = self.public_key.encrypt(1.0)
        ciphertext3 = ciphertext1 + ciphertext2
        decryption = self.private_key.decrypt(ciphertext3)
        self.assertEqual(-14.0, decryption)
    ...
    def testAddWithEncryptDecryptFloat1(self):
        ...
        ciphertext3 = ciphertext1 + ciphertext2
        decryption = self.private_key.decrypt(ciphertext3)
        self.assertEqual(16.0, decryption)
    ...
    def testMulWithEncryptedFloatAndEncodedNumber0(self):
        ...
        encoded1 = phe.encoding.EncodedNumber.encode(self.public_key, 1.38734864, precision=1e-2)
    ...
    def testMulRight(self):
        ...
        self.assertEqual(
            self.private_key.decrypt(ciphertext3), self.private_key.decrypt(ciphertext2)
        )
        self.assertEqual(3.14, self.private_key.decrypt(ciphertext2))

    def testMulZero(self):
        # Check that multiplying by zero does something sensible
        ciphertext1 = self.public_key.encrypt(3.0)
    ...
    def testDiv(self):
        ...
        self.assertEqual(2.0, self.private_key.decrypt(ciphertext3))

    def testAddWithEncryptedFloatAndEncodedNumber(self):
        # Add two floats with different precisions
        ciphertext1 = self.public_key.encrypt(0.1, precision=1e-3)
        encoded1 = phe.encoding.EncodedNumber.encode(self.public_key, 0.2, precision=1e-20)
