...
class ArithmeticTest(PaillierTest):
    def testMean(self):
        ...
        self.assertAlmostEqual(np.mean(self.vec4_1_non_neg), self.private_key.decrypt(e_mean4_1))

        emean4_2 = np.mean(self.e_vec4_2)
        self.assertAlmostEqual(np.mean(self.vec4_2), self.private_key.decrypt(emean4_2))

    def testDot(self):
        # Check that our dot product is as good as numpy's
        e_dot_4_2_4_1 = np.dot(self.e_vec4_2, self.vec4_1_non_neg)
        self.assertAlmostEqual(
            np.dot(self.vec4_2, self.vec4_1_non_neg), self.private_key.decrypt(e_dot_4_2_4_1)
        )
