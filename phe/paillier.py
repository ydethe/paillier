...
class PaillierPrivateKey(object):
    ...
    def raw_decrypt(self, ciphertext):
        ...
        decrypt_to_p = mulmod(
            self.l_function(powmod(ciphertext, self.p - 1, self.psquare), self.p), self.hp, self.p
        )
        decrypt_to_q = mulmod(
            self.l_function(powmod(ciphertext, self.q - 1, self.qsquare), self.q), self.hq, self.q
        )
...
class PaillierPrivateKeyring(Mapping):
    ...
    def add(self, private_key):
        ...
        if not isinstance(private_key, PaillierPrivateKey):
            raise TypeError(
                "private_key should be of type PaillierPrivateKey, " "not %s" % type(private_key)
            )
...
class EncryptedNumber(object):
    ...
    def decrease_exponent_to(self, new_exp):
        ...
        if new_exp > self.exponent:
            raise ValueError(
                "New exponent %i should be more negative than "
    ...
    def _add_scalar(self, scalar):
        ...
        """
        encoded = EncodedNumber.encode(self.public_key, scalar, max_exponent=self.exponent)
